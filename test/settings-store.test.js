...
test("settings UI exposes current LLM rewrite controls without legacy polish fields", () => {
  ...
  assert.equal(html.includes("推荐润写模型"), true);
  assert.equal(html.includes("gpt-5.1"), true);
  assert.equal(html.includes("MiniMax-M2.7"), true);
  assert.equal(html.includes("doubao-seed-1-6-250615"), true);
